...
{
    public class StateService(ILocalStorageService localStorage, ISessionStorageService sessionStorage)
    {
        public async Task SaveConfigAsync(PlanningConfig config)
        {
            await localStorage.SetItemAsync("planningConfig", config);
        }

        public async Task<PlanningConfig?> LoadConfigAsync()
        {
            return await localStorage.GetItemAsync<PlanningConfig>("planningConfig");
        }

        public async Task CacheHolidaysAsync(string key, IEnumerable<HolidayResponse> holidays)
        {
            await sessionStorage.SetItemAsync($"cache_holidays_{key}", holidays);
        }

        public async Task<IEnumerable<HolidayResponse>?> GetCachedHolidaysAsync(string key)
        {
            return await sessionStorage.GetItemAsync<IEnumerable<HolidayResponse>>($"cache_holidays_{key}");
        }
